...
SeastarClientTag* SeastarTagFactory::CreateSeastarClientTag(
  ...
  memcpy(&tag, p + 8, 8); //acquired the client-tag constructed before
  // ignore the method segment 4B in client side, and *env ignored either
  memcpy(&tag->status_, p + 20, 2);
  memcpy(&tag->resp_err_msg_len_, p + 22, 2);

  if (!tag->IsRecvTensor() && !tag->IsFuseRecvTensor()) {
...
SeastarServerTag* SeastarTagFactory::CreateSeastarServerTag(
  ...
  char* p = const_cast<char*>(header.get());
  auto* tag = new SeastarServerTag(seastar_channel, worker_service_);
