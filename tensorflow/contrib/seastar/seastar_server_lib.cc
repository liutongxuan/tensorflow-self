...
public:
  ...
  std::string GetSeastarIpPort(const std::string& job_name, int task_index) {
    ...
    std::vector<std::string> ip_port_pair = str_util::Split("", ":");
    CHECK_EQ(ip_port_pair.size(), 2);
    int grpc_port = -1;
    strings::safe_strto32(ip_port_pair[1], &grpc_port);
    CHECK_GT(grpc_port, 0);

    string ret;
    strings::Appendf(&ret, "%s:%d", ip_port_pair[0].c_str(), 49999);
    return ret;
...
//  seastar_bound_port_ = seastar_port_mgr_->GetLocalSeastarPort();
  seastar_bound_port_ = 49999;
  seastar_engine_ =
      new SeastarEngine(seastar_bound_port_, seastar_worker_service_);
