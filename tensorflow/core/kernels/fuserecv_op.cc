...
#include "tensorflow/core/kernels/fuserecv_op.h"
...
FuseRecvOp::FuseRecvOp(OpKernelConstruction* ctx) : AsyncOpKernel(ctx), fuse_count_(0) {
  // TODO(rangeng.llb): send devices are not single
  ...
  for (int i = 0; i < fuse_count_; ++i) {
    ...
    GetRendezvousKey(key_prefixs_[i], {0, 0}, parsed_keys_[i].raw());
    OP_REQUIRES_OK(ctx, Rendezvous::ParseKey(parsed_keys_[i].FullKey(), &parsed_keys_[i]));
  }
...
void FuseRecvOp::ComputeAsync(OpKernelContext* ctx, DoneCallback done) {
  ...
  Rendezvous::FuseDoneCallback done_cb = std::bind(
      [ctx](DoneCallback done,
        ...
        if (s.ok()) {
          ...
          // NOTE(rangeng.llb): is_output_dead means all the outputs is dead.
          ctx->all_output_dead(all_deads);
        }
        done();
      },
  ...
  if (!(frame_iter == FrameAndIter(0, 0))) {
    ...
    for (int i = 0; i < fuse_count_; ++i) {
      GetRendezvousKey(key_prefixs_[i], frame_iter, parsed_keys_[i].raw());
      OP_REQUIRES_OK_ASYNC(ctx,
                           Rendezvous::ParseKey(parsed_keys_[i].FullKey(), &parsed_keys_[i]),
                           done);
    }
  }
