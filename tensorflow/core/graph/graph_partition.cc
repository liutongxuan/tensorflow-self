...
// struct used to store fuse recv
...
typedef absl::flat_hash_map<DupRecvKey, FuseRecvInfo> GlobalDupFuseRecvTable;
...
// Set NodeDef's input
void SetInput(NodeDef* dst, int dst_slot, StringPiece src_name, int src_slot) {
  if (src_slot == Graph::kControlSlot) {
...
void SetFuseRecvAttrs(const PartitionOptions &opts,
  ...
  for (int i = 0; i < fuse_count; ++i) {
    tensor_names[i] =
        strings::StrCat("edge_", edges[i]->id(), "_", edges[i]->src()->name());
  }
...
Status DoFuseRecv(const PartitionOptions& opts, const GraphInfo& g_info,
  ...
  for (auto it = recvs.begin(); it != recvs.end(); ++it) {
    fuse_recv_infos.emplace_back(&(it->second));
//    edges.emplace_back((*fuse_recv_infos)[0].edge);
    edges.push_back((it->second)[0].edge);
  }
  ...
  for (auto fuse_recv_info : fuse_recv_infos) {
    ...
    bool init = true;
    int slot = 0;
    for (auto iter_inner = fuse_recv_info->begin();
         iter_inner != fuse_recv_info->end(); ++iter_inner) {
      if (init) {
...
void GetReadyNodes(std::unordered_set<Node*>& done,
    const std::string& loc, std::unordered_set<Node*>* node_set,
...
Status PartitionWithFuse(const PartitionOptions& opts, Graph* g,
  ...
  while (true) {
    ...
    for (auto nodes : ready_subgraph_nodes) {
      ...
      for (auto it = fuse_recv_groups.begin();
           it != fuse_recv_groups.end(); ++it) {
